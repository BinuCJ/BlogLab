...
namespace BlogLab.Web.Controllers
{
    ...
    public class PhotoController : ControllerBase
    {
        ...
        public async Task<ActionResult<int>> Delete(int photoId)
        {
            ...
            var foundPoto = await _photoRepository.GetAsync(photoId);

            if(foundPoto != null)
            {
                if(foundPoto.ApplicationUserId == applicationUserId)
                {
                    var blogs = await _blogRepository.GetAllByUserIdAsync(applicationUserId);

                    var usedInBlogs = blogs.Any(b => b.PhotoId == photoId);

                    if (usedInBlogs) return BadRequest("Cannot remove photo as it is being used in published Blog(s)");

                    var deleteResult = await _photoService.DeletePhotoAsync(foundPoto.PublicId);

                    if (deleteResult.Error != null) return BadRequest(deleteResult.Error.Message);

                    var affectedRows = await _photoRepository.DeleteAsync(foundPoto.PhotoId);

                    return Ok(affectedRows);
                }
                else
                {
                    return BadRequest("Photo are not uploaded by the current user.");
                }
            }
